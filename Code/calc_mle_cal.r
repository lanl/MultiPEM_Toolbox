...
# forward and error model parameters, with an option to include        #
# errors-in-variables for calibration data yields, based on            #
# calibration data.                                                    #
#                                                                      #
...
calc_mle_cal = function(p_cal,gdir,adir,f,nst=10,ncor=1,ci_lev=0.95,
                        igrad=TRUE,bfgs=TRUE,igrck=TRUE,g=NULL,
                        iresp=NULL,fp_fm=NULL,fopt_in=NULL,Xst=NULL,
                        cst=NULL,fopt_out=NULL,phen=NULL,pl="multicore")
{
  #
  # FUNCTION INPUTS
  #
  ...
  # ci_lev: confidence interval levels for calibration parameter
  #         inference
  ...
  if( is.null(Xst) ){
    ...
    for( ii in 1:nst ){
      nxst = 0
      if( ii > 1 ){ xst = runif(p_cal$nmpars, min=-2, max=2)
      } else { xst = numeric(p_cal$nmpars) }
      if( p_cal$ncalp > 0 ){
        if( ii == 1 && !is.null(cst) ){
          xst[nxst + 1:p_cal$ncalp] = cst
        }
  ...
  for( ii in 1:nst ){
    ...
    while( is.infinite(ll_full(xst, pc=p_cal)) ){
      xst = runif(p_cal$nmpars, min=-2, max=2)
      if( exists("eiv",where=p_cal,inherits=FALSE) && p_cal$eiv ){
        xst[nxst + 1:p_cal$nsource] = p_cal$eiv_w
      }
    }
  ...
  if( mle$convergence == 0 ){
    p_cal$mle_cal = unname(mle$par)
    opt_mle = make_opt(mle$par, p_cal)
    p_cal$opt_mle = opt_mle
...
ll_opt = function(xst,bfgs,pc)
{
  ...
  while( ii <= maxiter ){
    ...
    if( is(Catch$value,"list") ){
      ...
      xst = runif(pc$nmpars, min=-2, max=2)
      nxst = 0
