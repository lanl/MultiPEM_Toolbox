...
calc_mle = function(p_cal,gdir,adir,f,nst=10,ncor=1,ci_nev=0.95,
                    igrad=TRUE,bfgs=TRUE,igrck=TRUE,t_cal=NULL,g=NULL,
                    iresp=NULL,fp_fm=NULL,fopt_in=NULL,Xst=NULL,
                    tst=NULL,fopt_out=NULL,phen=NULL,pl="multicore")
{
  #
  # FUNCTION INPUTS
  #
  ...
  if( !igrad && exists("nev",where=p_cal,inherits=FALSE) &&
      p_cal$nev ){ p_cal$obs_info = obs_info }
  # support functions
  ...
  if( is.null(Xst) ){
    ...
    for( ii in 1:nst ){
      ...
      if( exists("eiv",where=p_cal,inherits=FALSE) && p_cal$eiv ){
        if( exists("nev",where=p_cal,inherits=FALSE) && p_cal$nev ){
          xst[p_cal$ntheta0+(1:p_cal$nsource)] = p_cal$eiv_w
        } else { xst[1:p_cal$nsource] = p_cal$eiv_w }
      }
      if( !is.null(fopt_in) && ii == 1 ){
        if( exists("nev",where=p_cal,inherits=FALSE) && p_cal$nev &&
            ("theta0" %in% names(opt[[1]])) ){
          xst[1:p_cal$ntheta0] = opt[[1]]$theta0
        }
        if( p_cal$ntheta0 > 0 || p_cal$nsource > 0 ){
          xst[-(1:(p_cal$ntheta0+p_cal$nsource))] =
  ...
  if( mle$convergence == 0 ){
    ...
    p_cal$iPrior = FALSE
    if( exists("nev",where=p_cal,inherits=FALSE) && p_cal$nev ){
      p_cal = print_ss(mle$par, p_cal, ci=ci_nev)
