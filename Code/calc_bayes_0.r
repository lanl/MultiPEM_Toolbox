...
calc_bayes_0 = function(p_cal,gdir,adir,nburn=10000,nmcmc=20000,nthin=1,
                        ncor=1,igrad=TRUE,igrck_pr=TRUE,igrck_po=TRUE,
                        bfgs=TRUE,itpr=FALSE,fpr_t=NULL,fgpr_t=NULL,
                        imcmc="FME",pl="multicore",ncor_smc=NULL,
                        lb_smc=NULL,ub_smc=NULL,t_cal=NULL)
{
  #
  # FUNCTION INPUTS
  #
  ...
  # ncor: number of cores for parallel MCMC chains or multiple
  #       imputation
  ...
  if( bfgs ){
    if( p_cal$opt_B ){
      map = optim(p_cal$mle, fn=lpost_0, gr=glpost_0, pc=p_cal,
                  method="L-BFGS-B",
                  lower=tlb, upper=tub,
    ...
    if( p_cal$opt_B ){
      map = optim(p_cal$mle, fn=lpost_0, pc=p_cal,
                  lower=tlb, upper=tub,
  ...
  if (imcmc == "RAM"){
    ...
    if( nimp == 1 ){
      ...
      si = sample(10000,ncor)
      ptm = proc.time()
      fram = lapply(1:ncor, function(qq) { future(MCMC(lpost_0,
                            nburn+round(nmcmc/ncor),init=xi[qq,],
  ...
  if (imcmc == "NUTS"){
    ...
    if( nimp == 1 ){
      ...
      ptm = proc.time()
      fham = lapply(1:ncor, function(qq) { future(NUTS(xi[qq,],
                            f=lpost_0,grad_f=glpost_0,
                            n_iter=nburn+round(nmcmc/ncor),
  ...
  if (imcmc == "SMC"){
    ...
    if( !is.null(ncor_smc) ){
      ...
      if( ncor > nimp ){ ncor = nimp }
      if( ncor_smc >= ncor_av ){ ncor = 1; ncor_smc = ncor_av; }
      ncor_tot = ncor*ncor_smc
      if( ncor_tot > ncor_av ){ ncor = ncor_av %/% ncor_smc }
    } else { ncor_smc = 1 }
    if( ncor == 1 ){ pl_1 = "sequential"
    } else { pl_1 = pl }
    if( ncor_smc == 1 ){ pl_2 = "sequential"
    } else { pl_2 = pl }
    plan(list(tweak(pl_1,workers=ncor),tweak(pl_2,workers=ncor_smc)))
