...
calc_bayes_cal = function(p_cal,gdir,adir,nst=10,nburn=10000,
                          ...
                          bfgs=TRUE,ibpr=FALSE,fpr_b=NULL,fgpr_b=NULL,
                          Xnom=NULL,imcmc="FME",pl="multicore")
{
  #
  # FUNCTION INPUTS
  #
  ...
  if( npars > p_cal$nmpars ){
    ...
    if( bfgs ){
      map = optim(p_cal$mle_cal, fn=lpost_full, gr=glpost_full,
                  pc=p_cal, method="BFGS",
                  control=list(fnscale = -1,maxit=1000))
    } else {
      map = optim(p_cal$mle_cal, fn=lpost_full, pc=p_cal,
  ...
  if( imcmc %in% c("RAM","FME") ){
    ...
    p_cal = p_cal$obs_info(p_cal, map, imle=FALSE)
  }
