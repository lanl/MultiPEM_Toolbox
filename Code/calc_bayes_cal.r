...
# errors-in-variables for calibration data yields.                     #
#                                                                      #
...
calc_bayes_cal = function(p_cal,gdir,adir,nst=10,nburn=10000,
                          nmcmc=20000,nthin=20,ncor_map=1,ncor_mc=1,
                          igrad=TRUE,igrck_pr=TRUE,igrck_po=TRUE,
                          bfgs=TRUE,ibpr=FALSE,icpr=FALSE,fpr_b=NULL,
                          fgpr_b=NULL,fpr_c=NULL,fgpr_c=NULL,Xnom=NULL,
                          imcmc="FME",pl="multicore")
{
  #
  # FUNCTION INPUTS
  #
  ...
  if( !exists("obs_info_cal",where=p_cal,inherits=FALSE) ){
    # R function to calculate observed information matrix
    source(paste(gdir,"/observed_information_cal.r",sep=""),local=TRUE)
    p_cal$obs_info_cal = obs_info_cal
  }
  ...
  Xst = matrix(p_cal$mle_cal,nrow=1)
  ...
  for( ii in 1:nst ){
    ...
    while( is.infinite(lpost_full(xst, pc=p_cal)) ){
      xst = runif(p_cal$npars, min=-2, max=2)
      if( exists("eiv",where=p_cal,inherits=FALSE) && p_cal$eiv ){
  ...
  if( imcmc %in% c("RAM","FME") ){
    ...
    p_cal = p_cal$obs_info_cal(p_cal, map, imle=FALSE)
    lambda = (2.38)^2/npars
...
lpo_opt = function(xst,bfgs,pc)
{
  ...
  while( ii <= maxiter ){
    ...
    if( is(Catch$value,"list") ){
      ...
      xst = runif(pc$npars,min=-2,max=2)
      nxst = 0
      if( pc$ncalp > 0 ){ nxst = nxst + pc$ncalp }
      if( exists("eiv",where=pc,inherits=FALSE) && pc$eiv ){
        xst[nxst + 1:pc$nsource] = pc$eiv_w
      }
