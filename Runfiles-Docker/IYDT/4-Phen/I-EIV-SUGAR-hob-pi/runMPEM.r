...
if( nev ){
  ...
  if( tsub ){
    ...
    tsub[[4]] = 1 # only log-yield for crater
  } else { tsub = NULL }
} else {
  dat_new = NULL
  theta_names = NULL
...
tmp = prepro(gen_dir,app_dir,dat_dir,dat_cal,Rh,pbeta,bopt=opt_B,
             ...
             fp_tr=tPars,tlb=lb_theta0,tub=ub_theta0,ndir=dat_new,
             tsub=tsub)
if( opt_B ){
  p_cal = tmp$p_cal
  t_cal = tmp$t_cal
...
if( fPars ){
  ...
  fPars[[3]]$pressure_scaling = 1/3
  fPars[[3]]$temp_scaling = 1/2
} else { fPars = NULL }
...
if( iBayes ){
  ...
  if( iBetaPrior ){
    ...
    prior_files_beta = paste(app_dir,"/lp_beta_s.r",sep="")
    if( igrad ){
      gr_prior_files_beta = paste(app_dir,"/glp_beta_s.r",sep="")
